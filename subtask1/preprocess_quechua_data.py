...
def process(args):
    ...
    for split in SPLITS:
        ...
        txt_file = os.path.join(data_dir, f'{split}/txt/{split}.que')
        with open(txt_file, 'r', encoding='utf-8') as f:
            dataset = [i.strip() for i in f.readlines()]
            print(split, len(dataset))
        print("Extracting log mel filter bank features...")
        wav_files = glob.glob(os.path.join(data_dir, f'{split}/wav/*'))
        for sample_id, filename in enumerate(sorted(wav_files, key=lambda x: int(x[-9:][:-4]))):
    ...
    for split in SPLITS:
        ...
        with open(txt_file, 'r', encoding='utf-8') as f:
            dataset = [i.strip() for i in f.readlines()]
        for idx, sentence in tqdm(enumerate(dataset)):
            # client_id, path, sentence = line.split('\t')[:3]
            sample_id = str(idx)
            utt = sentence
            spk_id = 'N/A'
